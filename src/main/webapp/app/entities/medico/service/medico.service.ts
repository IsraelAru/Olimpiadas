...
export class MedicoService {
  ...
  update(medico: IMedico): Observable<EntityResponseType> {
    return this.http.put<IMedico>(`${this.resourceUrl}/${getMedicoIdentifier(medico) as string}`, medico, { observe: 'response' });
  }

  partialUpdate(medico: IMedico): Observable<EntityResponseType> {
    return this.http.patch<IMedico>(`${this.resourceUrl}/${getMedicoIdentifier(medico) as string}`, medico, { observe: 'response' });
  }
